...
├── INSIGHTS.md
├── README.md
├── data_exploration_executed.ipynb
├── assets/
│   └── minimaps/
├── data/
│   ├── raw/
...
Minimap images for all 3 maps are included in `assets/minimaps/`. These are top-down views of each map.
...
# Read a single file
table = pq.read_table("data/raw/February_10/f4e072fa_b71aaad8.nakama-0")
df = table.to_pandas()
...
df_feb10 = load_day("data/raw/February_10")
```
